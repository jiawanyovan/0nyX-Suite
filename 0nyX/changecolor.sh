...
_folders="gtk gtk-3.0 gtk-2.0 xfwm4 xfdashboard-1.0 openbox-3 balou \
Extras"
for _folder in $_folders; do
	cd $_folder
	find "$basedir/$_folder" -type f -exec sed -i 's/#4f79bf/'$newcolor'/g' {} \;
	find "$basedir/$_folder" -type f -exec sed -i 's/#4F79bF/'$newcolor'/g' {} \;
	cd $basedir
done
#WINE
sed -i 's/79 121 191/'$r' '$g' '$b'/g' "$basedir/Extras/App_themes/WINE/0nyX.reg"

#change initial colors
sed -i 's#s/79 121 191#s/'$r' '$g' '$b'#g' "$basedir/changecolor.sh"
